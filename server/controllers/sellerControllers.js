...
const updateCategory = async (req, res) => {
  ...
  try {
    ...
    for (const key in categoryObject) {
      if (key !== "category_name" && key !== "category_id" && key !== "_id") {
...
const getNotifications = async (req, res) => {
  ...
  try {
    const notifications = await pool.query(
      "SELECT DISTINCT ON (order_id) notification_id, order_id, customer_name, order_date, status FROM notifications WHERE seller_id = $1 ORDER BY order_id, notification_id DESC;",
      [_id]
    );
...
const checkNotification = async (req, res) => {
  ...
  try {
    const notifications = await pool.query(
      "UPDATE notifications SET status = 'checked' WHERE seller_id = $1 AND order_id = $2 returning *;",
      [_id, order_id]
    );
    console.log("CHECKED: ", notifications.rows);
...
const deleteNotification = async (req, res) => {
  ...
  try {
    const notifications = await pool.query(
      "DELETE FROM notifications WHERE seller_id = $1 AND order_id = $2 RETURNING *;",
      [_id, order_id]
    );
